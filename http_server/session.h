...
const std::size_t MAX_BUFFER = 4096;
...
public:
	...
	inline void start();
public:
	void handle_read(const boost::system::error_code& err_code, std::size_t bytes_transferred);
	void handle_write(const boost::system::error_code& err_code);
    bool filter();
private:
	tcp::socket socket_;
	char        data_[MAX_BUFFER];
	int         recv_times_;
    set<string> filter_ip;
    void load_filter_ip();
};
...
void http_session::start()
{
	socket_.async_read_some(boost::asio::buffer(data_, MAX_BUFFER), \
			boost::bind(&http_session::handle_read, this,
